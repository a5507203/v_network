var NetworkVisualization = function ( editor ) {
   ...
   // this.texture = texture;
    ...
    d3.text("dataFile/SiouxFalls_node.txt", function(data) {
        ...
        d3.text("dataFile/nodes.csv", function(data2) {
            data2 = data2.replace(/,/g,'\t').replace(/\r\n/g,'\t;\r\n')+'\t;\r\n';
            console.log(data2);
         });

    });
    d3.text("dataFile/SiouxFalls_net3.txt", function(data2) {
        console.log(JSON.stringify(data2));

        d3.text("dataFile/networks3.csv", function(edges) {
            ...
            var firstNode  = '';
            var numberOfLinks = 0;
            for ( ; numberOfLinks < rows.length-1 ; numberOfLinks += 1) {
                var row = rows[numberOfLinks+1];
                colomn = row.split(',');
                if ( numberOfLinks == 0) firstNode = colomn[0];
                nodes.add(colomn[0]);
                nodes.add(colomn[1]);
            ...
            // if((JSON.stringify(data2)) == (JSON.stringify(content))) console.log('success')
            // console.log(content)
            // data2 = data2.replace(/,/g,'\t').replace(/\r\n/g,'\t;\r\n')+'\t;\r\n';
            // console.log(data2);
         });
    ...
    d3.text("dataFile/net.txt", function(data2) {
        d3.text("dataFile/trips3.csv", function(trips) {
...
NetworkVisualization.prototype = {
    ...
    readEdgesFromString : function ( data  ) {
        ...
        for ( i = startIndex ; i< lines.length ; i += 1 ) {
            ...
            if (line.length < 10 ) continue;
            // make the largest value to Math.tanh(0.7)
            var currCapcity = parseFloat(line[3]);
            ...
            Config.roadType = sortObject(Config.roadType);
